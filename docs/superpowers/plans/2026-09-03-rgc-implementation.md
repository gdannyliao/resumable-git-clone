...
15. **Task 13 已落地（6963a06，Approved）+ Task 14 验收标准**：RateLimited 走独立持久化 `rate_limits` 计数（连续、cap、serde-default）；FetchHook/SleepHook 测试缝隙（Send+Sync，默认透传）；冷却改为每次 429 都布防。**Task 14 必须**：(a) 全局 429 断路器——run 级总 429 计数（Arc<AtomicU32>），超阈值 bail 整个 run（"upstream rate-limited — rerun later"），同时作为并发减半的信号源；(b) 冷却等待改为循环重读（醒来后再查，直到归零）+ 每 worker 抖动，防 thundering herd；(c) state 改 Mutex 化（worker 当前 &mut State 是并行化真正障碍）；(d) Send+Sync 静态断言进测试。顺手项：成功步重置 rate_limits；storm 退避按 60<<rate_limits.min(2) 升级；冷却消费测试；as_secs 精确 Duration；poisoned lock 韧性；60 常量提取。

16. **Task 14 已落地（69b1700）**：jobs 并行 worker 共享 `Arc<Mutex<State>>`（claim 在锁内，write-ahead 语义不变）；全局 429 断路器（run 级 `total_rate_limits`，触发即收口 bail）；冷却等待循环重读 + 每 worker 抖动 + 精确 Duration；成功步重置连续限流计数。**语义影响**：rate_limits 在片 Done 后持久化为 0（成功重置），持久化计数仅在终态 Failed 片上可观测——失败注入测试的豁免/rebill 证据相应改为行为断言（attempts==0、rerun 存活 + fetch 调用数）。并行 e2e（jobs=3，等价 oracle 对照真 clone）、并发峰值>1、断路器风暴、冷却递减序列测试齐备。70 测试全绿。

17. **Task 14 审查修复已落地**：(a) **闸门死锁修复**——风暴停在软/硬阈值窗口且消退时 halving 永不恢复，被闸 worker 会在活跃 worker 清完片后永久滞留（run 在 join 挂死，审查实证复现）；修复为 Pending 清空即放行；(b) 闸门回归测试（修复前挂死）；(c) Done 片兜底重置 rate_limits（tag 片无逐步重试点，A.16 不变量补全）；(d) 冷却日志每次布防 episode 只记一条；(e) worker panic 合成失败消息进 run 报告；(f) `cooldown_secs` 提为配置字段（默认 COOLDOWN_SECS=60）；(g) 并发峰值测试窗口 200ms→500ms。71 测试全绿。

18. **Task 15 已落地（ee0d095）+ 审查修复**：子命令 clone/resume/status；A.8 契约全数落地（锁先于 load_plan/reconcile、孤儿/损坏 plan 大声报错绝不静默重规划、status 严格只读）。实现者额外修复潜伏 bug：相对 dest 路径使 transport 的 git argv 按错误 cwd 解析 → 永久重试假死（gitio::absolute_path 在 argv 边界收口 + 回归测试）。审查修复：**全新 dest 预检**（非空/既有 git 仓库先于 ls_remote 拒绝——否则 finalize 的 checkout -B 孤儿化既有提交、改写 origin url）；status 指纹失配警告；移除死依赖 indicatif。macOS flock drop-close 竞窗以显式 LOCK_UN 收口。83 测试全绿。

19. **Task 16 已落地（fc17f0e）+ 实测真 bug**：kill9 验收测试（轮询原始 state.json 首现"Done≥1 且 Running"即对真实二进制 SIGKILL → resume → plan.json 字节不变、已 Done 片不重做、等价 oracle 对照 git clone）。**测试立功揪出真产品 bug**：rgc 被 SIGKILL 时在途的 git 子进程（如 `clone --shared`）被 init 收养并跑完，片仓库停在"origin 指向克隆源（主仓库）"的中间态；健康探针只查 .git 一致性/alternates 不查 origin → resume 对主仓库 fetch 报 "couldn't find remote ref"。修复：健康路径幂等 `set-url origin` + `gc.auto` 收敛。CI（ubuntu/macos，build/test/clippy -D warnings/kill9 验收）+ README（用法/续传模型/错误模型/诚实限制清单）。默认套件 82 + kill9（ignored）1 = 83 全绿，clippy 干净。

20. **终审（SHIP WITH NOTES）收尾**：锁测试显式 unlock（macOS close-flock 异步窗导致 CI flake，必修项）；finalizer::verify 显式 shallow 断言（spec §3）；README 修正两处夸大表述（远端删 ref 的可恢复性、新增运行期磁盘 ≈2× 说明）；lib.rs 声明非公共 API。**入账偏差**：片仓库 run 期间不删（finalize 才清）——偏离 spec §4.4 磁盘模型，README 已如实记录，post-transport 删除列为后续项；Windows 仅编译验证弃做（README 声明不支持）；Ctrl-C 无显式 handler（进程组 SIGINT + Running 折返等效）。A.10a/A.11 片级跳过分诊保持 TODO（与 git clone 持平的失败语义）。
