...
- **片仓库初始化（通用规则）**：所有链式片仓库一律从主仓库 `git clone --shared` 初始化（而非空仓库），并把主仓库的 `refs/remotes/origin/*`、`refs/tags/*` 复制为片仓库的同名本地 refs——have 协商依据的是本地 refs，仅靠 alternates 对象不够；复制后（对象已在本地，引用复制近乎零成本）服务端只发该分支的独有增量，否则分支间共享的历史会被重复下载，传输量按分支数放大。§5 的崩溃恢复路径复用同一机制。
- 分支链的片仓库放在 `<dir>/.rgc/pieces/<ref>/` **持久化**（非系统临时目录），shallow 边界保存在其 `.git/shallow` 中。
- 每步完成后立即把增量搬运进主仓库并更新 state（含当前 depth），崩溃最多损失当前一步。
- 搬运的目标命名空间：链进行中写入 `refs/rgc/wip/<branch>`，链完成后才写正式 `refs/remotes/origin/<branch>` 并删除 wip 引用。原因：主仓库中途的 tip ref 若被其他片当作 have，服务端会按"拥有完整祖先"误判而静默跳过缺失历史（"撒谎的 have"）；`git clone --shared` 不复制 wip 命名空间，故 wip 引用不会泄漏进其他片的协商。
...
- **并行的真实作用域（诚实声明）**：链内串行意味着主干链是串行关键路径，单主干型仓库（如 linux 内核）的墙钟时间 ≈ 主干链时长，并行的加速上限取决于分支形态（典型 10–30%，不是 Nx）。`--jobs` 的价值在于：① 分支丰富型仓库的尾部片（其他分支增量、tags 批量）真正重叠执行；② 隐藏每片请求的服务端首字节延迟；③ 单通道挂起退避时不阻塞其余通道。**墙钟时间的主要杠杆是单片目标时长（§4.2），不是并发度。**
...
- 磁盘峰值 = 主仓库 + 在跑片（jobs 份；`--shared` 片仓库经 alternates 复用主仓库对象，增量远小于整片），每步搬运完即删，不积压。
...
- 深度维度的推测并行：预计算边界 SHA（forge API / commit 元数据先行）或推测性时间窗分片 + Finalizer 修复兜底，摆脱主干链的串行瓶颈。依赖 forge 特性、正确性风险较高，故不进 MVP。
