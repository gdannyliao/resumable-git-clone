...
| 网络错误 | 指数退避重试，片步长减半 |
| 致命（仓库损坏等） | 片终态 Failed，run 报告明细后退出；修复后重跑继续 |
| 远端 ref 消失（克隆中途删分支） | 与 `git clone` 同样直接失败（与 git 持平；尚无片级跳过分诊） |
| kill -9 / 断电 | 状态文件原子写，永不撕裂；重跑即续传（有验收测试） |

**运行期磁盘占用**：已完成的片仓库在 run 期间保留（完成后 finalize 时删除），
峰值 ≈ 主仓库 + 各片增量之和；GB 级仓库请预留约 2× 目标仓库的磁盘。
